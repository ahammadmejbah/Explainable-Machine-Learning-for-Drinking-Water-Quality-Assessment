...
st.markdown(
    ...
    .paper-section h3 { margin-top: 1.5rem; }
    ...
    .citation { background:white; border:1px solid var(--line); border-radius:6px; padding:1rem 1.1rem; font-size:.9rem; line-height:1.65; }
...
for column, value, label in zip(
    metric_columns,
    ["25", "2000-2026", "11", "3-fold"],
    ["Aggregated records", "Study period", "Compared classifiers", "Stratified validation"],
):
    with column:
...
st.markdown(
    """
The model target is derived from the transparent score: a record is labeled high risk when `Risk_Score >= 70`. This target should be understood as a constructed label, not an independently observed outcome. The workflow is intended to compare explainability patterns and generate hypotheses for future individual-sample studies.
...
5. Source notes, ranges, and percentages remain as reported and require review against original publications.
